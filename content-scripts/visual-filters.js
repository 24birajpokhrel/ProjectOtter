...
;(() => {
  ...
  // ── Dark Mode CSS ────────────────────────────────────────────────────────────
  // FIX SUMMARY:
  //   - Replaced `background-color: inherit` on `*` with an explicit dark value.
  //     Using `inherit` caused white/light-background cards and containers to
  //     bleed through because they resolved to their own non-dark parent colors.
  //   - Added `color: #e8e8e8 !important` to the `*` rule so ALL text is forced
  //     light, not just the explicitly listed tag types. This prevents dark-on-dark
  //     text on sites that use highly specific selectors.
  //   - Added explicit overrides for structural container elements (div, article,
  //     section, main, aside, header, footer, nav, card, etc.) that news/content
  //     sites commonly use with inline or component-scoped white backgrounds.
  //   - Slightly increased body text brightness (#e8e8e8 → #efefef) for better
  //     legibility at typical screen brightness levels.
  const DARK_MODE_CSS = `
      ...
      /* ── Universal override ──────────────────────────────────────────────────
         Force ALL elements to a dark background and light text.
         We exclude elements that use background-image (e.g. CSS hero/card images)
         so we don't paint a dark color over them and hide the image entirely. */
      html.accessilens-dark *:not([style*="background-image"]):not([class*="bg-"]) {
        background-color : #121212 !important;
        color            : #efefef !important;
        border-color     : #3a3a3a !important;
      }

      /* Elements with an inline background-image must keep their background
         visible — only reset color so text overlays on images remain readable. */
      html.accessilens-dark [style*="background-image"] {
        background-color : transparent !important;
        color            : #efefef !important;
      }

      /* ── Structural containers ───────────────────────────────────────────────
         News and content sites wrap articles in divs/sections with their own
         background declarations. Targeting these explicitly ensures no white
         cards survive the cascade. We exclude background-image holders. */
      html.accessilens-dark div:not([style*="background-image"]),
      html.accessilens-dark article:not([style*="background-image"]),
      html.accessilens-dark section:not([style*="background-image"]),
      html.accessilens-dark main:not([style*="background-image"]),
      html.accessilens-dark aside:not([style*="background-image"]),
      html.accessilens-dark header:not([style*="background-image"]),
      html.accessilens-dark footer:not([style*="background-image"]),
      html.accessilens-dark nav:not([style*="background-image"]),
      html.accessilens-dark figure:not([style*="background-image"]),
      html.accessilens-dark figcaption,
      html.accessilens-dark blockquote,
      html.accessilens-dark [class*="card"]:not([style*="background-image"]),
      html.accessilens-dark [class*="Card"]:not([style*="background-image"]),
      html.accessilens-dark [class*="panel"]:not([style*="background-image"]),
      html.accessilens-dark [class*="Panel"]:not([style*="background-image"]),
      html.accessilens-dark [class*="box"]:not([style*="background-image"]),
      html.accessilens-dark [class*="Box"]:not([style*="background-image"]),
      html.accessilens-dark [class*="container"]:not([style*="background-image"]),
      html.accessilens-dark [class*="Container"]:not([style*="background-image"]),
      html.accessilens-dark [class*="wrapper"]:not([style*="background-image"]),
      html.accessilens-dark [class*="Wrapper"]:not([style*="background-image"]) {
      ...
      /* ── Links ───────────────────────────────────────────────────────────────
         Keep links distinguishable and accessible against dark backgrounds. */
      html.accessilens-dark a,
      html.accessilens-dark a:visited {
        color : #7ab8f5 !important;
      }

      html.accessilens-dark a:hover {
        color : #a8d0ff !important;
      }

      /* ── Media — dim slightly to reduce glare ────────────────────────────────
         Images and videos render pixel content, not CSS colors, so background
         and color overrides must not apply to them or their direct wrappers.
         'transparent' on the element itself plus 'unset' on close ancestors
         prevents the dark container background bleeding into image bounds. */
      html.accessilens-dark img,
      html.accessilens-dark video,
      html.accessilens-dark canvas {
        background-color : transparent !important;
        filter           : brightness(0.85) !important;
      }

      /* Immediate wrappers of media should not impose a dark background —
         this is what causes the cropping/artifacting effect where the dark
         container color bleeds into the edges of images. */
      html.accessilens-dark img + *,
      html.accessilens-dark a:has(> img),
      html.accessilens-dark a:has(img),
      html.accessilens-dark figure,
      html.accessilens-dark picture,
      html.accessilens-dark [class*="image"],
      html.accessilens-dark [class*="Image"],
      html.accessilens-dark [class*="photo"],
      html.accessilens-dark [class*="Photo"],
      html.accessilens-dark [class*="thumb"],
      html.accessilens-dark [class*="Thumb"],
      html.accessilens-dark [class*="media"],
      html.accessilens-dark [class*="Media"],
      html.accessilens-dark [class*="hero"],
      html.accessilens-dark [class*="Hero"],
      html.accessilens-dark [class*="banner"],
      html.accessilens-dark [class*="Banner"],
      html.accessilens-dark [class*="poster"],
      html.accessilens-dark [class*="Poster"],
      html.accessilens-dark [class*="cover"],
      html.accessilens-dark [class*="Cover"],
      html.accessilens-dark [class*="featured"],
      html.accessilens-dark [class*="Featured"] {
        background-color : transparent !important;
      }

      /* SVG icons should remain visible but not treated as photos */
      html.accessilens-dark svg {
        background-color : transparent !important;
      }

      /* ── Form controls ───────────────────────────────────────────────────────
         Inputs need a slightly lighter dark surface to be visually distinct
         from the page background, aiding focus for users with cognitive load. */
      ...
      html.accessilens-dark input::placeholder,
      html.accessilens-dark textarea::placeholder {
        color : #888 !important;
      }

      /* ── Buttons ─────────────────────────────────────────────────────────────*/
      html.accessilens-dark button,
      html.accessilens-dark [role="button"],
      html.accessilens-dark [type="button"],
      html.accessilens-dark [type="submit"] {
        background-color : #2e2e2e !important;
        color            : #efefef !important;
        border           : 1px solid #555 !important;
      }

      /* ── Headings — slightly brighter for visual hierarchy ───────────────────*/
      html.accessilens-dark h1,
      html.accessilens-dark h2,
      html.accessilens-dark h3,
      html.accessilens-dark h4,
      html.accessilens-dark h5,
      html.accessilens-dark h6 {
        color : #ffffff !important;
      }

      /* ── Body text — readable mid-grey (avoids pure white fatigue) ───────────*/
      html.accessilens-dark p,
      html.accessilens-dark span,
      html.accessilens-dark li,
      html.accessilens-dark td,
      html.accessilens-dark th,
      html.accessilens-dark label,
      html.accessilens-dark caption,
      html.accessilens-dark dt,
      html.accessilens-dark dd,
      html.accessilens-dark cite,
      html.accessilens-dark time {
        color : #d8d8d8 !important;
      }

      /* ── Tables ──────────────────────────────────────────────────────────────*/
      html.accessilens-dark table {
        background-color : #1e1e1e !important;
      }

      html.accessilens-dark tr:nth-child(even) {
        background-color : #222222 !important;
      }

      /* ── Code blocks ─────────────────────────────────────────────────────────*/
      html.accessilens-dark code,
      html.accessilens-dark pre {
        background-color : #1e1e1e !important;
      ...
      /* ── Selection highlight ─────────────────────────────────────────────────*/
      html.accessilens-dark ::selection {
        background-color : #264f78 !important;
        color            : #ffffff !important;
      }

      /* ── Scrollbar (Chromium) ────────────────────────────────────────────────*/
      html.accessilens-dark ::-webkit-scrollbar {
        background-color : #1a1a1a !important;
      }

      html.accessilens-dark ::-webkit-scrollbar-thumb {
